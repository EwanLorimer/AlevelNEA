...
class foldersTableFunctions:
    ...
    def deleteFolder(folderID):
        #Deletes the folder in the datbase.      
        con = sqlite3.connect('users.db') #Establishes connection to the Database
        cur = con.cursor()
        print(folderID)
        #Return all article Titles
        #Get the articleID for each titles
        #Delete each article in folder to maintain referential integrity
        #Delete the folder
        articleTitles = articlesTableFunctions.returnArticleTitles(folderID)
        print(articleTitles)
        #Iterates through the article titles in folder and deletes them
        for i in range(len(articleTitles)):
            articleTitleList = articleTitles[i]
            
            #Gets article Title and ID to delete article
            articleTitle = generalDbFunctions.tupToStringSingle(articleTitleList,0)
            articleID = articlesTableFunctions.returnArticleID(articleTitle, folderID)
            #Deletes article
            cur.execute("DELETE FROM articles WHERE articleID='{}'".format(articleID))
        #Deletes folder when all a
        cur.execute("DELETE FROM folders WHERE folderID='{}'".format(folderID))
...
class articlesTableFunctions:
    ...
    def returnArticleID(articleName,articleFolderIn):
        #Return the primary key of an article
        con = sqlite3.connect('users.db') #Establishes connection to the Database
        cur = con.cursor()
        #Retrieve the article ID from the article in that folder and store as returnedID
        cur.execute("SELECT articleID FROM articles WHERE articleTitle='{}' AND articleFolderIn='{}'".format(articleName,articleFolderIn))
        returnedID = cur.fetchall()

        con.commit()
        cur.close()
        #If the ID has been found, it's length will be greater than zero. If so, return it.
        if len(returnedID) > 0:
            returnedID = returnedID[0]
            returnedID = returnedID[0]
            return returnedID
    ...
    def returnArticleKeyWords(articleID):
        #Return the keywords that an article has so they can be displayed on the screen.
        con = sqlite3.connect('users.db') #Establishes connection to the Database
        cur = con.cursor()
    ...
    def returnArticleNotes(articleID):
        #Return any notes that an article has so it can be displayed on screen
        con = sqlite3.connect('users.db') #Establishes connection to the Database
        cur = con.cursor()
    ...
    def returnArticleSummary(articleID):
        #Return the summary of the article so it can be displayed on screen.
        con = sqlite3.connect('users.db') #Establishes connection to the Database
        cur = con.cursor()
    ...
    def deleteArticle(articleID):
        ...
        #Deletes the article & all it's contents from the database
        cur.execute("DELETE FROM articles WHERE articleID='{}'".format(articleID))
        print(cur.fetchall())
        con.commit()
        cur.close()

articleID = 8650411817309
print("Summary of article: ",articleID,"\n")
print(articlesTableFunctions.returnArticleSummary(articleID))
