...
class foldersTableFunctions:
    def folderNameAvailable(folderName, username):
        ...
        con.commit()
        con.close()
        returnedFolders = generalDbFunctions.tupToString(returnedFolders)
        if len(returnedFolders) > 0:
            return False
        else:
            return True
    ...
    def returnFolders(username):
        ...
        cur.execute("SELECT folderName FROM folders WHERE folderOwner='{}'".format(username))#Retrieves all folder names from a given username.
        returnedFolders = cur.fetchall()
    ...
    def folderIDAvailable(newID):
        ...
        #Converts retrievedID into Integer so it can be checked
    ...
    def createFolder(folderName,folderOwner):
        folderID = funcs.createPk()
        if foldersTableFunctions.folderNameAvailable(folderName,folderOwner) == True:
            if foldersTableFunctions.folderIDAvailable(folderID) == True:

                con = sqlite3.connect('users.db') #Establishes connection to the Database
                cur = con.cursor() #Cursor tells database what to do
                
                cur.execute("INSERT INTO folders VALUES (?,?,?)",(folderName,folderOwner,folderID))
...
class articlesTableFunctions:
    def articleTitleAvailable(folderID,articleTitle):
        ...
        con.commit()
        cur.close()
        returnedTitle = generalDbFunctions.tupToString(returnedTitle)
        if returnedTitle == articleTitle:
            return False
        else:
            return True
    ...
    def returnArticleTitles(folderID):
        ...
        cur.execute("SELECT articleTitle FROM articles WHERE articleFolderIn='{}'".format(folderID))
        returnedArticles = cur.fetchall()
...
folderID = generalDbFunctions.returnContentsofTable('folders')
