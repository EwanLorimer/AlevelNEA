...
def folderPage():
    ...
    if "sessionUsername" in session:
        ...
        if request.method == "POST":
                ...
                #Post development changes: Have a button which deletes the folder
                else:
                    #Take the value of the selected folder and go into it
                    folderSelected = request.form["folderSelected"]
                    if request.form.get('delete') == 'Delete Folder':
                        #RUN FUNCTION WHICH DELETES FOLDER
                        folderID = foldersTableFunctions.returnFolderID(folderSelected, sessionUsername)
                        print("Delete folder: ",folderID)
                        
                        foldersTableFunctions.deleteFolder(folderID)
                        return redirect(url_for("folderPage"))
...
def articlesPage():
    if "sessionFolderSelected" in session:
        ...
        if request.method == "POST":
            ...
            #Else, user has selected an article, take user to that article or delete that article (Post development)
            else:
                
                articleName = request.form["articleSelected"]
                #Post development: If the button to delete is clicked, delete articles
                if request.form.get('delete') == 'Delete Article':
                        #RUN FUNCTION WHICH DELETES FOLDER
                        articleID = articlesTableFunctions.returnArticleID(articleName, folderID)
                        
                        articlesTableFunctions.deleteArticle(articleID)
                        #Return articles page with updates user articles
                        return redirect(url_for("articlesPage"))

                session["sessionFolderID"] = folderID
...
def notesPage():
    if "sessionArticleName" in session:
        articleName = session["sessionArticleName"]
        
        folderID = session["sessionFolderID"]
        articleID = articlesTableFunctions.returnArticleID(articleName,folderID)

        #Retrieve the keywords,notes and summary for the article  from the database
        articleKeyWords = articlesTableFunctions.returnArticleKeyWords(articleID)
        if len(articleKeyWords) == 0:
            articleKeyWords = "Key words go here"

        articleNotes = articlesTableFunctions.returnArticleNotes(articleID)
        if len(articleNotes) == 0:
            articleNotes = "Notes go here"

        articleSummary = articlesTableFunctions.returnArticleSummary(articleID)
        if len(articleSummary) == 0:
            articleSummary = "Write your summary here"
        ...
        if request.method == "POST":
            ...
            articlesTableFunctions.editArticle(articleID, title, keyWords, notes, summary)
            session["sessionArticleName"] = articleName
            return redirect(url_for("notesPage"))
    return render_template('notesPage.html',articleName=articleName, articleKeyWords=articleKeyWords, articleNotes=articleNotes, articleSummary=articleSummary)
#Runs the program
